def data_path(
    ...
    *,
    ...
    ----------
    path : None | str
        Location of where to look for the brainstorm (bst_resting) dataset.
        If None, the environment variable or config parameter
        ``MNE_DATASETS_BRAINSTORM_DATA_PATH`` is used. If it doesn't exist, the
        "~/mne_data" directory is used. If the brainstorm (bst_resting) dataset
        is not found under the given path, the data
        will be automatically downloaded to the specified folder.
    force_update : bool
        Force update of the brainstorm (bst_resting) dataset even if a local copy exists.
        Default is False.
    update_path : bool | None
        If True (default), set the ``MNE_DATASETS_BRAINSTORM_DATA_PATH`` in mne-python
        config to the given path. If None, the user is prompted.
    download : bool
        If False and the brainstorm (bst_resting) dataset has not been downloaded yet,
